(ns player-rankings.handler
  (:require [compojure.core :refer [defroutes routes]]
            ...
            [player-rankings.routes.player :refer [player-routes]]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "player_rankings.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (cronj/start! session/cleanup-job)
  (timbre/info "\n-=[ player-rankings started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "player-rankings is shutting down...")
  (stop-nrepl)
